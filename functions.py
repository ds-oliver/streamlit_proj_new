...
def style_dataframe(df, selected_columns):
    cm_sns_bwr = cm.get_cmap('bwr')
    object_cmap = cm.get_cmap('bwr')  # Choose a colormap for object columns
...
def get_color_from_palette(value, palette_name='bwr'):
...
def style_dataframe_v2(df, selected_columns):
    object_cmap = mpl_cm.get_cmap('bwr')
    ...
    for col in df.columns:
        ...
        if len(unique_values) <= 3:
            constant_colors = [get_color(i / 2, mpl_cm.get_cmap('bwr')) for i in range(len(unique_values))]
            ...
            max_val = df[col].max()
            range_val = max_val - min_val
            styled_df[col] = df[col].apply(lambda x: get_color((x - min_val) / range_val, mpl_cm.get_cmap('bwr')))
