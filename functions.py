...
def style_dataframe(df, selected_columns):
    cm_sns_gist_heat = cm.get_cmap('gist_heat')
    object_cmap = cm.get_cmap('gist_heat')  # Choose a colormap for object columns
    ...
    for col in df.columns:
        ...
        if df[col].dtype in [np.float64, np.int64] and col in selected_columns:
            ...
            styled_df[col] = df[col].apply(lambda x: get_color((x - min_val) / range_val, cm_sns_gist_heat))
...
def get_color_from_palette(value, palette_name='gist_heat'):
...
def style_dataframe_v2(df, selected_columns):
    object_cmap = mpl_cm.get_cmap('gist_heat')
    ...
    for col in df.columns:
        ...
        if len(unique_values) <= 3:
            constant_colors = [get_color(i / 2, mpl_cm.get_cmap('gist_heat')) for i in range(len(unique_values))]
            ...
            max_val = df[col].max()
            range_val = max_val - min_val
            styled_df[col] = df[col].apply(lambda x: get_color((x - min_val) / range_val, mpl_cm.get_cmap('gist_heat')))
...
#     Team_cmap = plt.cm.get_cmap('gist_heat')
#     styled_df = pd.DataFrame('', index=df.index, columns=df.columns)
...
def style_tp_dataframe_custom(df, selected_columns, custom_cmap_name="gist_heat"):
